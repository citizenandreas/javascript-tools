...
(function() {
    ...
    _t.MultiCache = function(_getMethod, _settings) {
        ...
        var _AddBuffer = function(buffer) {
            
        };

        var _getContainingBuffer = function(pos) {
            for (var i = 0, j = _buffers.length; i < j; i++) {
                var buffer = _buffers[i];
                var bufferFrom = buffer.GetFrom();
                var bufferTo = buffer.GetTo();
                if (pos >= bufferFrom && pos <= bufferTo) {
                    return buffer;
                }
            }
        ...
        var _GetData = function(from, to, callback) {
            var data = new Array();
            var fromBuffer = null, toBuffer = null;

            if (from > to) {
                throw "\"From\" value must be higher or equal to the \"to\" value.";
            }
            ...
            for (var i = 0, j = _buffers.length; i < j; i++) {
                ...
                var bufferFrom = buffer.GetFrom();
                var bufferTo = buffer.GetTo();
                if (from >= bufferFrom && to <= bufferTo) {
                    buffer.GetData(from, to, function(data) {
                        callback(data);
                    });
                    return;
                }

                if (from >= bufferFrom && from <= bufferTo) {
                    fromBuffer = buffer;
                }

                if (to >= bufferFrom && to <= bufferTo) {
                    toBuffer = buffer;
                }
            }

            var fbfrom, fbto, tbfrom, tbto;
            if (fromBuffer !== null && toBuffer !== null) {
                fbfrom = fromBuffer.GetFrom();
                fbto = fromBuffer.GetTo();
                tbfrom = toBuffer.GetFrom();
                tbto = toBuffer.GetTo();

                if (fbto === (tbfrom - 1)) {
                    //Buffers run together
                    fromBuffer.GetData(from, fbto, function(fromData) {
                        toBuffer.GetData(tbfrom, to, function(toData) {
                            var data = fromData.concat(toData);
                            callback(data);
                        });
                    });
                } else {
                    ...
                    fromBuffer.GetData(from, fbto, function(fromData) {
                        toBuffer.GetData(tbfrom, to, function(toData) {
                            var midBuffer = new _t.AutoLoadingBuffer(fbto + 1,tbfrom -1,g_loadModes.Center,_getMethod, { ChunkSize : _config.ChunkSize });
                            _AddBuffer(midBuffer);
                            midBuffer.GetData(tbfrom, to, function(midData) {
                                var data = fromData.concat(midData,toData);
                                callback(data);
                            });
